...
class ContactController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $list = \DB::table($this->config['list_table'])->where('cid',$id)->orderBy('id','asc')->get();

        $view = \View::make("admin.{$this->config['folder']}.edit", compact('data' , 'list' , 'action'))->with('config', $this->config)->render();
    ...
    public function add($id)
    {
        ...
        // 寫入資料表的內容
        $values = \Request::except('_token', '_method', 'action', 'upload');
     ...
     * Excel 行動 選擇器
     */
    public function excel($action)
    {
        switch ($action) {
            case 'download_all':
                $this->excel_download_all();
                break;          
            default:
                return back()->withErrors('無對應的方法,動作參數：' . $action)->send();
                break;
        }
    }
     ...
     * Excel 多筆 導出 (橫式)
     */
    private function excel_download_all()
    {
        return with(new \App\Exports\ContactExport)->download()->send();
    }
}