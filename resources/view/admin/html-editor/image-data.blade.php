@forelse ($data as $value)
	<div class="item" ondblclick="select_pic('{{ storage_path($value['pic']) }}')" draggable="true" ondragstart="dragStart(event, {{ $value['id'] }})">
	...
	@endif
		...
		<div>
			簡介:
			<input type="text" onblur="pics_upd({{ $value['id'] }}, 'name', this.value)" value="{{ $value['name'] }}" placeholder="圖片簡介">
		</div>
	</div>
@empty
	<div>暫無任何圖片</div>
