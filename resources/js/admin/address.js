...
var country_arr = [ '台北市', '基隆市', '新北市', '連江縣', '宜蘭縣',  '新竹市', '新竹縣', '桃園市', '苗栗縣', '台中市', '彰化縣', '南投縣', '雲林縣', '澎湖縣', '金門縣', '花蓮縣', '嘉義市', '嘉義縣', '台南市', '高雄市', '屏東縣', '台東縣'];
...
//初始化
function country_init(country, town, dom_country, dom_town, dom_zip) {
 	if (dom_country == undefined) {
		var dom_country = $('select[name="country"]');
	}

	if (dom_town == undefined) {
		var dom_town = $('select[name="town"]');
	}

	if (dom_zip == undefined) {
		var dom_zip = $('input[name="zip"]');
	}

	dom_country.unbind('change');
	dom_town.unbind('change');

	var country_str = '<option value="" hidden>選擇縣市</option>';
	var town_str = '<option value="" hidden>選擇區域</option><option value="" disabled>請先選擇縣市</option>';
	var zip_val = '';

	for(i = 0; i < country_arr.length; i++) {
		if (country_arr[i] != country){
			country_str += '<option value="'+country_arr[i]+'">'+country_arr[i]+'</option>';
		} else {
			country_str += '<option value="'+country_arr[i]+'" selected>'+country_arr[i]+'</option>';

			if (town_arr.hasOwnProperty(country_arr[i])) {
				town_str = '<option value="" hidden>選擇區域</option>';

				$.each(town_arr[country_arr[i]] , function(key, val) {
					if (key != town) {
		        		town_str += '<option value="'+key+'">'+key+'</option>';
					} else {
						town_str += '<option value="'+key+'" selected>'+key+'</option>';
						zip_val = val;
					}
		        });
			}
	...
	dom_country.html(country_str);
	dom_town.html(town_str);
	dom_zip.val(zip_val);

	dom_country.bind('change', function() {
		let  country_selected = dom_country.find('option:selected').val();

		let str = '<option value="" hidden>選擇區域</option>';

		if (town_arr.hasOwnProperty(country_selected)) {
			$.each(town_arr[country_selected] , function(key, val) {
	...
	dom_town.bind('change', function() {
		let country_selected = dom_country.find('option:selected').val();
		let town_selected = dom_town.find('option:selected').val();
		let zip = town_arr[country_selected][town_selected];
